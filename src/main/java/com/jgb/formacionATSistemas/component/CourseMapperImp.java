...
public class CourseMapperImp implements CourseMapper{
	...
	public Course dtoToModel(CourseDTO dto) {
		...
		Optional<Course> course = courseService.findById(dto.getIdCourse());

		List<Optional<User>> usersOpt = dto.getUsers().stream().map(u -> userService.findById(u)).collect(Collectors.toList());
		List<User> users = usersOpt.stream().map(Optional::get).collect(Collectors.toList());
		
		List<Optional<Quiz>> quizOpt = dto.getQuiz().stream().map(a -> quizService.findById(a)).collect(Collectors.toList());
		List<Quiz> answers = quizOpt.stream().map(Optional::get).collect(Collectors.toList());
		course.get().setQuiz(answers);
		course.get().setUser(users);
		return course.get();
	}
